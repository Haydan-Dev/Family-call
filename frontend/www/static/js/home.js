document.addEventListener('DOMContentLoaded', () => {
  ...
  async function initPushNotifications() {
    ...
    if (!(window.Capacitor && window.Capacitor.isNativePlatform())) {
      console.log('[PushNotifications] Skipped — not running on native platform.');
      return;
    }

    const PushNotifications = window.Capacitor.Plugins.PushNotifications;
    if (!PushNotifications) {
      console.error('[PushNotifications] Plugin not found. Did you run `npx cap sync`?');
      return;
    }
    ...
    PushNotifications.addListener('registration', async (token) => {
      console.log('🎯 NATIVE_FCM_TOKEN:', token.value);

      // Phase 2: Vault the FCM token in MongoDB via backend API
      try {
        const res = await authFetch(`${BASE_URL}/users/update_fcm_token`, {
          method: 'PATCH',
          body: JSON.stringify({ fcm_token: token.value })
        });

        if (res.ok) {
          console.log('✅ Token Vaulted in DB!');
        } else {
          const err = await res.json().catch(() => ({}));
          console.error('❌ FCM token vault failed:', res.status, err.detail || err);
        }
      } catch (error) {
        console.error('❌ FCM token vault network error:', error);
      }
    });

    // ❌ Critical error during token fetch
